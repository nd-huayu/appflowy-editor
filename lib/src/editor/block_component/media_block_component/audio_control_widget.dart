...
///
/// {@endtemplate}
Widget DefaultDesktopAudioControls(VideoState state) {
  return const VideoControlsThemeDataInjector(
    child: _MaterialDesktopVideoControls(),
  );
}

/// [MaterialDesktopAudioControlsThemeData] available in this [context].
MaterialDesktopAudioControlsThemeData _theme(BuildContext context) =>
    FullscreenInheritedWidget.maybeOf(context) == null
        ? MaterialDesktopVideoControlsTheme.maybeOf(context)?.normal ??
            kDefaultMaterialDesktopAudioControlsThemeData
        : MaterialDesktopVideoControlsTheme.maybeOf(context)?.fullscreen ??
            kDefaultMaterialDesktopAudioControlsThemeDataFullscreen;

/// Default [MaterialDesktopAudioControlsThemeData].
const kDefaultMaterialDesktopAudioControlsThemeData =
    MaterialDesktopAudioControlsThemeData();

/// Default [MaterialDesktopAudioControlsThemeData] for fullscreen.
const kDefaultMaterialDesktopAudioControlsThemeDataFullscreen =
    MaterialDesktopAudioControlsThemeData();

/// {@template material_desktop_video_controls_theme_data}
///
/// Theming related data for [DefaultDesktopAudioControls]. These values are used to theme the descendant [DefaultDesktopAudioControls].
///
/// {@endtemplate}
class MaterialDesktopAudioControlsThemeData {
  // BEHAVIOR
  ...
  const MaterialDesktopAudioControlsThemeData({
    ...
    this.bottomButtonBarMargin = const EdgeInsets.symmetric(horizontal: 0.0),
    this.buttonBarHeight = 20.0,
    ...
    this.seekBarMargin =
        const EdgeInsets.only(left: 0, top: 6, bottom: 0, right: 36),
    this.seekBarHeight = 4,
    this.seekBarHoverHeight = 5.6,
    this.seekBarContainerHeight = 20,
    this.seekBarColor = const Color(0xFFF6F6F6),
    this.seekBarHoverColor = const Color(0x3DFFFFFF),
    this.seekBarPositionColor = const Color(0xFF3261E4),
    this.seekBarBufferColor = const Color(0x3DFFFFFF),
    this.seekBarThumbSize = 0.0,
    this.seekBarThumbColor = const Color(0xFFFF0000),
    this.volumeBarColor = const Color(0xFFF6F6F6),
    this.volumeBarActiveColor = const Color(0xFF3261E4),
    this.volumeBarThumbSize = 0.0,
    this.volumeBarThumbColor = const Color(0xFFF6F6F6),
    this.volumeBarTransitionDuration = const Duration(milliseconds: 150),
    this.shiftSubtitlesOnControlsVisibilityChange = true,
  });

  /// Creates a copy of this [MaterialDesktopAudioControlsThemeData] with the given fields replaced by the non-null parameter values.
  MaterialDesktopAudioControlsThemeData copyWith({
    bool? displaySeekBar,
    ...
    return MaterialDesktopAudioControlsThemeData(
...
///
/// Inherited widget which provides [MaterialDesktopAudioControlsThemeData] to descendant widgets.
///
/// {@endtemplate}
class MaterialDesktopVideoControlsTheme extends InheritedWidget {
  final MaterialDesktopAudioControlsThemeData normal;
  final MaterialDesktopAudioControlsThemeData fullscreen;
    ...
    extends State<MaterialDesktopPlayOrPauseButton>
{
  ...
  Widget build(BuildContext context) {
    return GestureDetector(
      onTap: () => controller(context).player.playOrPause(),
      child: Image.asset(
        isToggled
            ? 'assets/images/audio_block_pause.png'
            : 'assets/images/audio_block_play.png',
              ...
              GestureDetector(
                ...
                child: volume == 0.0
                    ? (widget.volumeMuteIcon ??
                        Image.asset('assets/images/audio_block_mute.png',
                            package: 'appflowy_editor',
                            width: 16,
                            height: 16,
                            fit: BoxFit.cover))
                    : (widget.volumeHighIcon ??
                        Image.asset('assets/images/audio_block_volume_max.png',
                            package: 'appflowy_editor',
                            width: 16,
                            height: 16,
                            fit: BoxFit.cover)),
    ...
    return RichText(
      text: TextSpan(
        ...
        children: <TextSpan>[
          ...
          TextSpan(
            text: '/',
            style: TextStyle(color: Color(0xFFCCCCCC)), // 设置 "/" 的颜色为 #ccc
          ),
          TextSpan(
            text: ' ${duration.label(reference: duration)}',
            style: TextStyle(color: Color(0xFFCCCCCC)), // 设置之后内容的颜色为 #ccc
          ),
        ],
      ),
    );
